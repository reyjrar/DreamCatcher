...
BEGIN
	-- Find this Question
	select id into out_question_id from question
		where class=in_class and type=in_type and name=in_name
	limit 1;  -- When we find it, stop.

	IF NOT FOUND THEN
		-- create it
		insert into question ( class, type, name )
			values ( in_class, in_type, in_name );
		select currval('question_id_seq') into out_question_id;
	END IF;

	-- Link the Query / Question
	insert into meta_question ( query_id, question_id ) values ( in_query_id, out_question_id );
	-- Update the question tracking data
	update question set last_ts=NOW(), reference_count=reference_count+1 where id=out_question_id;
